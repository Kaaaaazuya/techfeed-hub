...
describe('TechfeedStack', () => {
  ...
  describe('IAM Permissions', () => {
    test('should grant ECS task access to secrets', () => {
      ...
      const hasSecretsAccess = Object.values(policies).some((policy: { Properties?: { PolicyDocument?: { Statement?: any[] } } }) => {
        const statements = policy.Properties?.PolicyDocument?.Statement || [];
        return statements.some((stmt: Record<string, any>) => stmt.Action?.includes('secretsmanager:GetSecretValue'));
      });
    ...
    test('should grant Lambda access to secrets', () => {
      ...
      const lambdaRoles = Object.values(roles).filter((role: Record<string, any>) =>
        role.Properties?.AssumeRolePolicyDocument?.Statement?.some((stmt: Record<string, any>) =>
    ...
    test('should create Lambda execution role with VPC permissions', () => {
      const roles = template.findResources('AWS::IAM::Role');
      const lambdaRole = Object.values(roles).find((role: Record<string, any>) =>
        role.Properties?.AssumeRolePolicyDocument?.Statement?.some(
          (stmt: Record<string, any>) => stmt.Principal?.Service === 'lambda.amazonaws.com',
        ),
      ) as any;
